...
model <- glm(
  high_risk ~ BrainVital8_alignedQ + age + sex + BMI + edu + smoke +
    alcohol.user + income_ratio + Hypertension + DM + phq_scores,
  data = d,
  family = binomial
) %>% reg_table(round = 2)
...
model <- glm(
  high_risk ~ BrainVital8_aligned + age + sex + BMI + edu + smoke +
    alcohol.user + income_ratio + Hypertension + DM + phq_scores,
  data = d,
  family = binomial
) %>% reg_table(round = 3)
